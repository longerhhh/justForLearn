...
*/
        ...
        for (int i = 0; i < nums.length - 2; i++) {
            ...
            while (s < e) {
                if (nums[s] == b) {
                    ...
                    c = nums[e];
                    int target = -b - c;
                    System.out.println("s=" + s + ",e=" + e + ",sum=" + target + ",start=" + b + ",end=" + c + ", i=" + i);
                    //添加到列表，移动左指针
                    if (a == target) {
    ...
    /**
     * 还是双指针方法
     * ！！！！！ 这不叫双指针，这只是披着狼皮的羊 ！！！！ 实际上还是三重循环，所以效率低
     *
     * @param nums
     * @return
     */
...
//            System.out.println("l="+l+" r="+r+" d="+d+" m="+m);
            for (int i = m; i <= r - 1 && i > l; ) {
//                System.out.println("m="+m+" nums[i]="+nums[i]+" d="+d+" i="+i);
                if (nums[i] == d) {
            ...
            while (--r > l + 1 && nums[r] == right) ;
            if (r == l + 1) {
                r = lastIndex;
                while (++l < r - 1 && nums[l] == left) ;
            }
        }
        return result;
    }
    ...
    /**
     * 先排序，从排序后的数组两头开始找，判断第三个数是否也在数组中且下标不等于这两个下标，
     * 注意第三个数可能有重复的值，一般找到的是第一个数的下标，所以如果下标不满足，需要往后看一下有没有其他可用的下标，
     * 直到算出来的第三个数大小超过查找的左边界或右边界
     * 之后作边界右移一位，如果左边界数是连续重复的数字，则移动到第一个不重复的位置，
     * 右边界也是，如果重复，向左移动到第一个不重复的位置
     * 继续循环查找，直到查找范围只剩三个数，结束查找，返回结果
     * <p>
     * ！！还超时！！
     * 优化indexOf，大于时break，勉强能过，不超时了
     * <p>
     * ！！！！！ 这不叫双指针，这只是披着狼皮的羊 ！！！！ 实际上还是三重循环，所以效率低
     *
     * @param nums
...
//        System.out.println(Arrays.toString(nums));
        int lastIndex = nums.length - 1;
        int l = 0, r = lastIndex;
        int left, right, d, index;
        while (r - l > 0) {
            left = nums[l];
            right = nums[r];
            d = -left - right;
            index = indexOf(nums, l + 1, r - 1, d);
//            System.out.println("left=" + left + " right=" + right + " d=" + d + " index=" + index);
...
//        System.out.println("start: " + start+" end: " + end+" n: " + n);
            ...
            for (int j = i + 1; j < nums.length; j++) {
                ...
                for (int k = j + 1; k < nums.length; k++) {
                    ...
                    if (sum(t) == 0) {
                        ...
                        List<Integer> tc = (List<Integer>) t.clone();
                        Collections.sort(tc);
                        if (!contains(result, tc)) {
                            result.add(tc);
                        }
                    }
                }
    ...
    private boolean contains(List<List<Integer>> r, List<Integer> list) {
